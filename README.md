...
Next will be to generate a OAuth client, under the credentials tab again, click create credentials and select "OAuth Client ID." Set the application type to "Web Application" and name it something unique, mine is "Calendar Client Elliott." There is two very important steps left that we must hit, firstly add a Authorized redirect URI that is exactly as follows "http://localhost:8080/" this will ensure we can create our token. The last step is to hit create and very importantly download the JSON file. We must rename the JSON file to "credentials.json" and add it to the route of your directory, this will be ignored by git.
